...
m_description = """This model consists of shipment from sources to destinations with multi commodities.
                ...
                customer, Production capacity  of commodities in units at each factory and Demand of commodities in 
                units at each customer. 
...
rows = html.Div([
    html.Div([
        dbc.Row([dbc.Col(dbc.Alert(html.H4("Sigle-Stage-Multi-Commodity Transportation Model"),color="primary"),width=8),
        ...
        if len(list_of_contents) == 3 and mask:
            ...
            for i, j in dict_cont.items():
                if "cost" in i:
                    ...
                    h1 = df_cost[headers[0]].fillna(value="same")
                    h1_np = np.array(h1)
                    ...
                    df_cost.drop([headers[0], headers[1]], axis=1, inplace=True)
