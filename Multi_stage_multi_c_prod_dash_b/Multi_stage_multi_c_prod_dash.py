...
class TransPortationModel:
    ...
    def create_data_frames(self):
        ...
        prod_cost = self.cost_for_production[:p_size].reshape((p_size, 1))
        ...
        self.df_prod_cost = pd.DataFrame(prod_cost, p_index, ["Production Cost"])
        df = pd.DataFrame(p_index, p_index, [self.index_headers[1]])
    ...
    def solve_problem(self):
        ...
        c1 = self.cost_for_f_to_cust.reshape((p_size, no_of_c))
        c2 = self.cost_for_f_to_wh.reshape((p_size, no_of_w))
        ...
        cost_function = np.append(cost_function, self.cost_for_wh_to_cust)
...
tool_tip = """Seven separate excel files(namely 1-shipment_cost_f_to_c.xls, 2-shipment_cost_f_to_w.xls,
              3-shipment_cost_w_to_c.xls, 4-production_cost.xls, 5-demand.xls, 6-f_capacity.xls and 7-w_capacity.xls) 
              should be selected for shipment costs,production cost,customers demand, factories capacity and warehouses capacity. 
...
m_description = """This model consists of multi stage of transportation, with multi commodities, like shipment from 
                sources to destinations, sources to warehouses and warehouses to destinations.The objective of the 
                solution is to minimize the costs of shipping goods from sources to destinations with considering 
                production cost, while not exceeding the supply available from each source, meeting the demand of each
                destination and keeping inflow and outflow of warehouses same and not exceeding  capacity.
                ...
                warehouse to each customer,Production cost of commodities at each factories, Production capacity  of 
                commodities in units at each factory ,Storage capacity  of commodities in units at each warehouse and 
                Demand of commodities in units at each customer. Two options are available for giving input the first
                one is using interactive sheets the other is uploading data by excel sheets."""

rows = html.Div([

    html.Div([
    dbc.Row([dbc.Col(dbc.Alert(html.H4("Multi-Stage-Multi-Commodity Transportation Cum Production Model"),
                                                                                              color="primary"),width=9),
    dbc.Col(dbc.Button("Model Description", id="Model-Description-B", n_clicks=0, color="info"),
                                                                            width={"size": "auto", "offset": 1})]),
    dbc.Modal([dbc.ModalHeader("Model Description"),
    dbc.ModalBody(m_description,style={"color": "blue"}),
    dbc.ModalFooter(dbc.Button("Close", id="Modal-Description-close", n_clicks=0))], id="Modal-Description")]),
    ...
    dbc.Row([dbc.Col(
    dash_table.DataTable(id="table-7",
                             columns=[],
                             data=[],
                             editable=True,
                             export_format="xlsx",
...
@app.callback(
    [dash.dependencies.Output("table-0", "data"),
     ...
     dash.dependencies.Output("table-7", "data"),
     dash.dependencies.Output("table-7", "columns"),
...
def size_enter(n1,n2,list_of_contents,n3,v1,v2,v3,v4,select,p_list,list_of_names,is_open):
    ...
    if "dim-submit" in changed_id and select == "yes":
        ...
        mask = np.array(temp)
        mask = mask.all()
        if len(list_of_contents) == 7 and mask:
            ...
            msg = """Seven separate excel files should be selected.
...
@app.callback(
    ...
    dash.dependencies.State("table-7", "data"),
    dash.dependencies.State("scale", "value"),
    dash.dependencies.State("modal-1", "is_open")
     ])
def solve_model(n1, n2, cost_f_to_c, cost_w_to_c, cost_f_to_w, prod_cost, demand, f_capa, w_capa, u, is_open):
    if n1 == 0 and n2 == 0:
        raise PreventUpdate
    else:
        if u == None:
        ...
        df4 = pd.DataFrame(prod_cost)
        headers = df4.columns.values.tolist()
        df4.drop([headers[0], headers[1]], axis=1, inplace=True)
        problem.cost_for_production = np.array(df4).flatten().astype(np.float)
        df5 = pd.DataFrame(demand)
        headers = df5.columns.values.tolist()
        df5.drop(headers[0], axis=1, inplace=True)
        df5 = df5.transpose()
        problem.c_demand_constraint = np.array(df5).flatten().astype(np.float)
        df6 = pd.DataFrame(f_capa)
        headers = df6.columns.values.tolist()
        df6.drop([headers[0], headers[1]], axis=1, inplace=True)
        problem.f_capacity_constraint = np.array(df6).flatten().astype(np.float)
        df7 = pd.DataFrame(w_capa)
        headers = df7.columns.values.tolist()
        df7.drop(headers[0], axis=1, inplace=True)
        df7 = df7.transpose()
        problem.w_capacity_constraint = np.array(df7).flatten().astype(np.float)
        ...
        mask4 = np.isnan(problem.cost_for_production)
        mask4 = np.invert(mask4).all()
        mask5 = np.isnan(problem.c_demand_constraint)
        mask5 = np.invert(mask5).all()
        mask6 = np.isnan(problem.f_capacity_constraint)
        mask6 = np.invert(mask6).all()
        mask7 = np.isnan(problem.w_capacity_constraint)
        mask7 = np.invert(mask7).all()
        result1 = html.Div()
        table1 = html.Div()
        table2 = html.Div()
        table3 = html.Div()

        if mask1 and mask2 and mask3 and mask4 and mask5 and mask6 and mask7:
            if not is_open:
